...
/*!
 * \file auto_scheduler/search_policy/tile_policy_rules.h
 * \brief Rules for generating the tilees, sampling the initial population, and mutating the
 * population in TilePolicy.
 */

#ifndef TVM_AUTO_SCHEDULER_SEARCH_POLICY_TILE_POLICY_RULES_H_
#define TVM_AUTO_SCHEDULER_SEARCH_POLICY_TILE_POLICY_RULES_H_
...
class TilePolicyNode;

/********** Tile Generation Rule **********/

/*! \brief The base class for derivation rules used in the tile generation. */
class TileGenerationRule {
 public:
   ...
   * \param policy The TilePolicyNode of this rule, some information may be used during
   ...
   */
  virtual ConditionKind MeetCondition(const TilePolicyNode& policy, const State& state,
   ...
   */
  virtual std::vector<std::pair<State, int>> Apply(const TilePolicyNode& policy,
...
#define DEFINE_TILE_GENERATION_RULE(rule_name)                                                 \
  class rule_name : public TileGenerationRule {                                                \
   public:                                                                                       \
    ConditionKind MeetCondition(const TilePolicyNode& policy, const State& state,              \
                                int stage_id) const final;                                       \
    std::vector<std::pair<State, int>> Apply(const TilePolicyNode& policy, const State& state, \
                                             int stage_id) const final;                          \
 ...
 * the next stage. */
DEFINE_TILE_GENERATION_RULE(RuleSkipStage);
 ...
 */
DEFINE_TILE_GENERATION_RULE(RuleAlwaysInline);

/*! \brief The rule that performs multi-level tiling. */
DEFINE_TILE_GENERATION_RULE(RuleMultiLevelTiling);

/*! \brief The rule that performs multi-level tiling and fuses later consumers. */
DEFINE_TILE_GENERATION_RULE(RuleMultiLevelTilingWithFusion);
...
DEFINE_TILE_GENERATION_RULE(RuleAddCacheRead);

/*! \brief The rule that adds a cache write stage. */
DEFINE_TILE_GENERATION_RULE(RuleAddCacheWrite);

/*! \brief The rule that adds rfactor stage. */
DEFINE_TILE_GENERATION_RULE(RuleAddRfactor);
...
DEFINE_TILE_GENERATION_RULE(RuleSimplifyComputeWithConstTensor);

/*! \brief The rule that use cross thread reduction for GPU. */
DEFINE_TILE_GENERATION_RULE(RuleCrossThreadReduction);
...
DEFINE_TILE_GENERATION_RULE(RuleSpecialComputeLocationGPU);

/********** Init Population **********/

/*! \brief The base class for rules used to annotate the tilees to get the initial population. */
class PopulationGenerationRule {
 public:
  ...
  /*!
   * \brief Apply function of this rule.
   * \param policy The TilePolicyNode of this rule, some member may get changed during the
   ...
   */
  virtual ResultKind Apply(TilePolicyNode* policy, State* state,
...
#define DEFINE_INIT_POPULATION_RULE(rule_name)                                                    \
  class rule_name : public PopulationGenerationRule {                                             \
   public:                                                                                        \
    ResultKind Apply(TilePolicyNode* policy, State* state, std::mt19937* rand_gen) const final; \
  };
...
#endif  // TVM_AUTO_SCHEDULER_SEARCH_POLICY_TILE_POLICY_RULES_H_
