...
/*!
 * \file auto_scheduler/search_policy/tile_policy_rules.cc
 * \brief Rules for generating the tilees, sampling the initial population, and mutating the
 * population in TilePolicy.
 */
...
TileGenerationRule::ConditionKind RuleSkipStage::MeetCondition(const TilePolicyNode& policy,
                                                                 const State& state,
                                                                 int stage_id) const {
...
std::vector<std::pair<State, int>> RuleSkipStage::Apply(const TilePolicyNode& policy,
                                                        const State& state, int stage_id) const {
  return {std::make_pair(state, stage_id - 1)};
}

/********** RuleAlwaysInline **********/
inline bool ShouldAlwaysBeInlined(const TilePolicyNode& policy, const State& state,
                                  int stage_id) {
...
TileGenerationRule::ConditionKind RuleAlwaysInline::MeetCondition(const TilePolicyNode& policy,
                                                                    const State& state,
                                                                    int stage_id) const {
...
std::vector<std::pair<State, int>> RuleAlwaysInline::Apply(const TilePolicyNode& policy,
                                                           const State& state, int stage_id) const {
  State tmp_s = state;
...
TileGenerationRule::ConditionKind RuleMultiLevelTiling::MeetCondition(
    const TilePolicyNode& policy, const State& state, int stage_id) const {
  return NeedsMultilevelTiling(policy.search_task, state, stage_id)
             ? ConditionKind::kApplyAndSkipRest
             : ConditionKind::kSkip;
}

std::vector<std::pair<State, int>> RuleMultiLevelTiling::Apply(const TilePolicyNode& policy,
                                                               const State& state,
                                                               int stage_id) const {
  const std::string& multi_level_tiling_structure =
      IsGPUTask(policy.search_task)
          ? GetStringParam(policy.params, TileParamKey::MultiLevelTiling::gpu_structure)
          : GetStringParam(policy.params, TileParamKey::MultiLevelTiling::cpu_structure);
  std::cout << "multi_level_tiling_structure " << multi_level_tiling_structure << "\n";
...
TileGenerationRule::ConditionKind RuleMultiLevelTilingWithFusion::MeetCondition(
    const TilePolicyNode& policy, const State& state, int stage_id) const {
...
TileGenerationRule::ConditionKind RuleAddCacheRead::MeetCondition(const TilePolicyNode& policy,
                                                                    const State& state,
                                                                    int stage_id) const {
  const SearchTask& task = policy.search_task;
...
std::vector<std::pair<State, int>> RuleAddCacheRead::Apply(const TilePolicyNode& policy,
                                                           const State& state, int stage_id) const {
  const SearchTask& task = policy.search_task;
  const std::set<int>& consumers = GetConsumers(task, state, stage_id);
  State tmp_s = state;
...
TileGenerationRule::ConditionKind RuleAddCacheWrite::MeetCondition(const TilePolicyNode& policy,
                                                                     const State& state,
                                                                     int stage_id) const {
...
std::vector<std::pair<State, int>> RuleAddCacheWrite::Apply(const TilePolicyNode& policy,
                                                            const State& state,
                                                            int stage_id) const {
  State tmp_s = state;
...
TileGenerationRule::ConditionKind RuleAddRfactor::MeetCondition(const TilePolicyNode& policy,
                                                                  const State& state,
                                                                  int stage_id) const {
...
std::vector<std::pair<State, int>> RuleAddRfactor::Apply(const TilePolicyNode& policy,
                                                         const State& state, int stage_id) const {
  ...
  // TODO(merrymercy): We can do more analysis here to generate less and more efficient tilees.
...
TileGenerationRule::ConditionKind RuleSimplifyComputeWithConstTensor::MeetCondition(
    const TilePolicyNode& policy, const State& state, int stage_id) const {
...
TileGenerationRule::ConditionKind RuleCrossThreadReduction::MeetCondition(
    const TilePolicyNode& policy, const State& state, int stage_id) const {
...
std::vector<std::pair<State, int>> RuleCrossThreadReduction::Apply(const TilePolicyNode& policy,
                                                                   const State& state,
                                                                   int stage_id) const {
  const SearchTask& task = policy.search_task;
  State tmp_s = state;
...
TileGenerationRule::ConditionKind RuleSpecialComputeLocationGPU::MeetCondition(
    const TilePolicyNode& policy, const State& state, int stage_id) const {
...
PopulationGenerationRule::ResultKind InitFillTileSize::Apply(TilePolicyNode* policy, State* state,
                                                             std::mt19937* rand_gen) const {
  SplitFactorizationMemo split_memo;
  int max_innermost_split_factor =
      GetIntParam(policy->params, TileParamKey::max_innermost_split_factor);
  ...
  for (size_t step_id = 0; step_id < (*state)->transform_steps.size(); ++step_id) {
    if (auto ps = (*state)->transform_steps[step_id].as<SplitStepNode>()) {
      std::cout << "[InitFillTileSize] iter_id: " << ps->iter_id << " lengths: " << ps->lengths.size() << "\n";
      ...
      if (all_defined) {
        std::cout << "[InitFillTileSize] all_defined\n";
        continue;
      }
      ...
      std::cout << "[InitFillTileSize] candidate_lengths\n";
      for (auto i : candidate_lengths) {
        std::cout << i << "\t";
      }
      std::cout << "\n";
...
PopulationGenerationRule::ResultKind InitChangeComputeLocation::Apply(
    TilePolicyNode* policy, State* state, std::mt19937* rand_gen) const {
  if (GetIntParam(policy->params, TileParamKey::disable_change_compute_location)) {
    return ResultKind::kValid;
  }
...
PopulationGenerationRule::ResultKind InitParallel::Apply(TilePolicyNode* policy, State* state,
                                                         std::mt19937* rand_gen) const {
  std::function<void(const TilePolicyNode&, State*, int stage_id, int iter_offset)>
      annotate_parallel;
  annotate_parallel = [&annotate_parallel](const TilePolicyNode& policy, State* state,
...
PopulationGenerationRule::ResultKind InitUnroll::Apply(TilePolicyNode* policy, State* state,
                                                       std::mt19937* rand_gen) const {
...
PopulationGenerationRule::ResultKind InitVectorization::Apply(TilePolicyNode* policy,
                                                              State* state,
                                                              std::mt19937* rand_gen) const {
  for (size_t stage_id = 0; stage_id < (*state)->stages.size(); ++stage_id) {
    const Stage& stage = (*state)->stages[stage_id];
    ...
    while (num_fusible < static_cast<int>(stage->iters.size())) {
      ...
      if (cum_length_prod > GetIntParam(policy->params, TileParamKey::max_vectorize_size)) {
        break;
      }
...
PopulationGenerationRule::ResultKind InitThreadBind::Apply(TilePolicyNode* policy, State* state,
                                                           std::mt19937* rand_gen) const {
  ...
  for (size_t stage_id = 0; stage_id < (*state)->stages.size(); ++stage_id) {
    ...
    if (NeedsMultilevelTiling(policy->search_task, *state, stage_id)) {
      const Stage& stage = (*state)->stages[stage_id];
      std::cout << "[InitThreadBind] compute_at: " << (int)stage->compute_at << "\n";
...
PopulationGenerationRule::ResultKind MutateTileSize::Apply(TilePolicyNode* policy, State* state,
                                                           std::mt19937* rand_gen) const {
  int max_innermost_split_factor =
      GetIntParam(policy->params, TileParamKey::max_innermost_split_factor);
...
PopulationGenerationRule::ResultKind MutateAutoUnroll::Apply(TilePolicyNode* policy, State* state,
                                                             std::mt19937* rand_gen) const {
...
PopulationGenerationRule::ResultKind MutateComputeLocation::Apply(TilePolicyNode* policy,
                                                                  State* state,
                                                                  std::mt19937* rand_gen) const {
  if (GetIntParam(policy->params, TileParamKey::disable_change_compute_location)) {
    return ResultKind::kInvalid;
...
PopulationGenerationRule::ResultKind MutateParallel::Apply(TilePolicyNode* policy, State* state,
                                                           std::mt19937* rand_gen) const {
